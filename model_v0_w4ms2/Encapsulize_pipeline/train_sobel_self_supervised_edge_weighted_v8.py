...
CHECKPOINT_NAME = 'fullpipeline_v8_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v8_best.pth'
AUTO_MASK_EPS = 1e-3
...
def photometric_loss(img_src, img_tgt, flow, valid=None, auto_mask=True, eps=1e-3):
    """
    Monodepth2-style auto-masked photometric loss.
    A pixel contributes only if warping improves over identity reconstruction.
    """
    ...
    if auto_mask:
        ...
        auto_mask_map = (photo_warped + eps < photo_identity).float()
    else:
        auto_mask_map = torch.ones_like(photo_warped)

    if valid is not None:
        if valid.ndim == 4:
            valid = valid[:, 0]
        valid = valid.float()
        auto_mask_map = auto_mask_map * valid

    return (photo_warped * auto_mask_map).sum() / (auto_mask_map.sum() + 1e-6)
...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        loss_self = photometric_loss(img_src, img_tgt, pred, valid, auto_mask=True, eps=AUTO_MASK_EPS)
