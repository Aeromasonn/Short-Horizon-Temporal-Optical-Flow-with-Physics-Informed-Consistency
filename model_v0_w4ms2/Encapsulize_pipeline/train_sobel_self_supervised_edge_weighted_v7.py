...
CHECKPOINT_NAME = 'fullpipeline_v7_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v7_best.pth'
...
def photometric_confidence_map(img_src, img_tgt, flow, alpha=10.0, texture_floor=0.2):
    """
    Build a soft confidence map for self-supervision.
    - High photometric residual -> low confidence
    - Low-texture regions are downweighted instead of being treated as fully reliable
    """
    warped_src = warp_image(img_src, flow)
    photo_res = (warped_src - img_tgt).abs().mean(dim=1, keepdim=True)
    photo_conf = torch.exp(-alpha * photo_res.detach())

    texture = normalize_map(sobel_grad_map(img_src)).detach()
    texture_conf = texture_floor + (1.0 - texture_floor) * texture

    return photo_conf * texture_conf



def photometric_loss(img_src, img_tgt, flow, valid=None, confidence=None):
    warped_src = warp_image(img_src, flow)
    photo = charbonnier_loss(warped_src - img_tgt).mean(dim=1, keepdim=True)

    weight = torch.ones_like(photo)
    ...
    return (photo * weight).sum() / (weight.sum() + 1e-6)
...
def edge_aware_smoothness_loss(flow, image, smoothness_cap=0.6):
    ...
    base_weight_x = torch.exp(-torch.mean(torch.abs(img_dx), dim=1, keepdim=True))
    base_weight_y = torch.exp(-torch.mean(torch.abs(img_dy), dim=1, keepdim=True))

    # Cap the maximum smoothness weight so low-texture regions (e.g. sky)
    # are not forced to be overly smooth.
    weight_x = torch.clamp(base_weight_x, max=smoothness_cap)
    weight_y = torch.clamp(base_weight_y, max=smoothness_cap)
...
class Trainer:
    def __init__(
        ...
        self.photo_alpha = photo_alpha
        self.texture_floor = texture_floor
        self.smoothness_cap = smoothness_cap
    ...
    def train_step(self, batch):
        ...
        self_conf = photometric_confidence_map(
            img_src,
            img_tgt,
            pred,
            alpha=self.photo_alpha,
            texture_floor=self.texture_floor,
        )
        loss_self = photometric_loss(img_src, img_tgt, pred, valid, confidence=self_conf)
        loss_temp = temporal_loss(pred_flows)
        loss_smooth = edge_aware_smoothness_loss(pred, img_src, smoothness_cap=self.smoothness_cap)
