...
CHECKPOINT_NAME = 'fullpipeline_v4_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v4_best.pth'
...
def boundary_edge_loss(flow, image, valid=None, edge_thresh=0.1, use_soft_mask=True):
    """
    Enforce flow boundaries to align with image boundaries.
    flow:  [B, 2, H, W]
    image: [B, 3, H, W]
    valid: [B, 1, H, W] or [B, H, W] or None
    """
    img_edge = normalize_map(sobel_grad_map(image))
    flow_edge = normalize_map(sobel_grad_map(flow))

    hard_mask = (img_edge > edge_thresh).float()
    mask = img_edge.detach().clamp(0.0, 1.0) if use_soft_mask else hard_mask
    ...
    target = img_edge.detach()
    loss = torch.abs(flow_edge - target) * mask
...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        loss_boundary = boundary_edge_loss(pred, img_src, valid=valid, edge_thresh=0.1, use_soft_mask=True)
