...
# v14 update:
# 1) keep v12 refined decoder
# 2) keep boundary supervision
# 3) remove sky loss entirely for clean ablation
# 4) keep the rest of the v12 loss balance unchanged
PHOTO_GRAD_THRESH = 0.03
CENSUS_PATCH = 7

SAVE_DIR = Path('checkpoints')
SAVE_DIR.mkdir(parents=True, exist_ok=True)
CHECKPOINT_NAME = 'fullpipeline_v14_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v14_best.pth'
AUTO_MASK_EPS = 1e-3
...
# v14 structural change:
# keep decoder_v1 with a full-resolution residual refinement head from v12.
...
def photometric_loss_v14(
    ...
    """
    v14 keeps the v12 robust self-supervised formulation:
...
class Trainer:
    ...
    def train_step(self, batch, epoch_idx=0):
        ...
        loss_self = photometric_loss_v14(
            img_src,
            img_tgt,
            pred,
            valid=valid,
...
def save_epoch_checkpoint(save_dir, epoch, trainer, optimizer, stats=None):
    """
    Save checkpoint at a specific epoch number (e.g., epoch 200).
    """
    filename = f'checkpoint_epoch_{epoch}.pth'
    save_path = save_dir / filename
    save_checkpoint(save_path, epoch, trainer, optimizer, stats=stats)
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch + 1) == 200:
        save_epoch_checkpoint(SAVE_DIR, epoch + 1, trainer, optimizer, stats=avg)
