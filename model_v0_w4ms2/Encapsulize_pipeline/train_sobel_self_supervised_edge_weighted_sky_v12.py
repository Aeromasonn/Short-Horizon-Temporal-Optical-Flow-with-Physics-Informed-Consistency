...
# v12 update:
# 1) reduce temporal pressure to avoid over-smoothing collapse
# 2) strengthen supervised learning at motion boundaries
# 3) replace sky smoothness with dominant-background-consistency in confident sky
# 4) keep sky prior weak and warm it up gradually
LAMBDA_SKY = 0.01
...
CHECKPOINT_NAME = 'fullpipeline_v12_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v12_best.pth'
AUTO_MASK_EPS = 1e-3
...
# v12 structural change:
# use decoder_v1 with a full-resolution residual refinement head.
...
def photometric_loss_v12(
    ...
    """
    v12 keeps the v11 robust self-supervised formulation:
...
def flow_grad_map(flow):
    """Per-sample normalized gradient magnitude of a 2-channel flow field."""
    fx = flow[:, 0:1]
    fy = flow[:, 1:2]
...
def boundary_weighted_epe_loss(pred, gt, image, valid=None, image_scale=1.0, flow_scale=2.0):
    """
    Extra supervised boundary loss: prioritize pixels near image edges and GT motion boundaries.
    This directly counters the blurry low-frequency solution seen in v9/v10.
    """
    epe = torch.norm(pred - gt, dim=1, keepdim=True)
    img_edge = normalize_map(sobel_grad_map(image)).detach()
...
def dominant_background_flow_loss(flow, image, grad_thresh=0.04, top_ratio=0.38, erosion_kernel=11):
    """
    Encourage coherent sky/background motion without forcing sky flow to zero.
    A dominant background flow vector is estimated inside the confident sky mask,
    and each sky pixel is softly encouraged to stay close to that dominant motion.
    """
    sky_mask = build_high_confidence_sky_mask(
        image,
        grad_thresh=grad_thresh,
...
class Trainer:
    ...
    def train_step(self, batch, epoch_idx=0):
        ...
        loss_self = photometric_loss_v12(
            img_src,
            img_tgt,
            pred,
            valid=valid,
        ...
        loss_boundary = boundary_weighted_epe_loss(
            ...
            img_src,
            valid=valid,
            image_scale=self.edge_weight_scale,
            flow_scale=2.0,
        )
