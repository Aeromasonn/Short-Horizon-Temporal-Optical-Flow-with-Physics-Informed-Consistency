...
CHECKPOINT_NAME = 'fullpipeline_v6_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v6_best.pth'
...
def warp_flow(flow_to_warp, ref_flow):
    return warp_image(flow_to_warp, ref_flow)



def forward_backward_consistency_mask(flow_fw, flow_bw, alpha=0.01, beta=0.5, valid=None):
    """
    flow_fw: [B, 2, H, W]  forward flow
    flow_bw: [B, 2, H, W]  backward flow defined on target image
    valid:   [B, 1, H, W] or [B, H, W] or None
    returns: [B, 1, H, W] binary consistency mask
    """
    flow_bw_warped = warp_flow(flow_bw, flow_fw)
    fb_error = torch.sum((flow_fw + flow_bw_warped) ** 2, dim=1, keepdim=True)
    fb_thresh = alpha * (
        torch.sum(flow_fw ** 2, dim=1, keepdim=True)
        + torch.sum(flow_bw_warped ** 2, dim=1, keepdim=True)
    ) + beta
    mask = (fb_error < fb_thresh).float()
...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        rev_src_idx = (pred_flows.shape[1] - 1 - src_idx).long()
        pred_flows_rev = out_rev['flows']
        pred_bw = select_gt_flow(pred_flows_rev, rev_src_idx)

        fb_mask = forward_backward_consistency_mask(
            pred,
            pred_bw,
            alpha=self.fb_alpha,
            beta=self.fb_beta,
        ...
        loss_self = photometric_loss(img_src, img_tgt, pred, fb_mask)
