...
CHECKPOINT_NAME = 'fullpipeline_v9_latest.pth'
BEST_CHECKPOINT_NAME = 'fullpipeline_v9_best.pth'
AUTO_MASK_EPS = 1e-3
...
def photometric_loss_v9(
    ...
    """
    Upgraded self-supervised loss for v9:
...
def low_gradient_flow_penalty(flow, image, threshold=0.04):
    """
    Penalize large flow magnitude in low-gradient regions such as sky / flat background.
    This gives an explicit constraint where GT supervision and photometric supervision
    are both weak or absent.
    """
    edge_map = normalize_map(sobel_grad_map(image)).detach()
    flow_mag = torch.norm(flow, dim=1, keepdim=True)
    penalty_weight = (threshold - edge_map).clamp(min=0.0) / (threshold + 1e-6)
    return (flow_mag * penalty_weight).mean()
...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        loss_sky = low_gradient_flow_penalty(
            pred,
            img_src,
            threshold=self.sky_grad_thresh,
        )
