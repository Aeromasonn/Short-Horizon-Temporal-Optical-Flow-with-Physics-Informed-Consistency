...
class LearnedUpsampleRefiner(nn.Module):
    """Feature-guided high-resolution refinement after bilinear flow upsampling.

    This is not full UPFlow, but it avoids directly trusting a plain resized decoder
    output. The bilinear-upsampled flow provides a coarse field; upsampled features
    predict a residual correction at image resolution.
    """
    def __init__(self, hidden_ch=64):
        ...
        self.refine = nn.Sequential(
            ...
            nn.Conv2d(hidden_ch // 2, 2, kernel_size=3, padding=1),
        )

    def forward(self, feat, flow, upsample):
        b, _, h, w = flow.shape
        H, W = h * upsample, w * upsample
        ...
        # Flow is in pixel units of the low-resolution grid, so scale the vector values.
        flow_up[:, 0] *= W / w
        flow_up[:, 1] *= H / h

        delta = self.refine(torch.cat([feat_up, flow_up], dim=1))
...
class FlowDecoderSingle(nn.Module):
    ...
    Input:
        z: [B, C, h, w]
        flow_prev: optional previous full-resolution flow [B,2,H,W]

    Output:
        high-resolution residual flow [B,2,H*upsample,W*upsample]
    """
    def __init__(self, in_ch=128, hidden_ch=64, upsample=8, use_prev_flow=True):
        ...
        self.up_refiner = LearnedUpsampleRefiner(hidden_ch=hidden_ch)
    ...
    def forward(self, z, flow_prev=None):
        ...
        flow_high = self.up_refiner(feat, flow_low, self.upsample)
        return flow_high
