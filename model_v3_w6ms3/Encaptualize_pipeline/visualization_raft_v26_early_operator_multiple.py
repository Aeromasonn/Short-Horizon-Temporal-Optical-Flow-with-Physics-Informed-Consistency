...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize all 3 adjacent v26 UNO + convex-upsample decoder predictions with RAFT-style color mapping.")
...
def build_modules(cfg: Dict[str, Any], device: torch.device) -> Dict[str, torch.nn.Module]:
    ...
    uno_in_channels = num_pairs * (cfg["model"].get("uno_input_ch", cfg["model"]["pair_embed_ch"]) + 6)
    ...
    latent_head = UNOLatentResidualHead(
        ...
        latent_ch=cfg["model"].get("uno_latent_ch", cfg["model"]["pair_embed_ch"]),
        num_pairs=num_pairs,
    ).to(device)
...
def forward_pipeline(modules: Dict[str, torch.nn.Module], cfg: Dict[str, Any], imgs: torch.Tensor, valid: torch.Tensor) -> Dict[str, torch.Tensor]:
    ...
    if flow_inits is None:
        raise RuntimeError("v26 early-operator requires predict_flow_init=True in the pair encoder.")

    # EARLY OPERATOR INTEGRATION:
    # UNO refines pairwise image-flow embeddings before MotionBranchCNN/Fusion.
    valid_ds = None
    if cfg["model"].get("uno_use_valid_mask", False):
    ...
    motion_feats = modules["motion_branch"](refined_pair_feats)
    fused_seq = modules["fusion"](visual_feats, motion_feats)

    dec_out = modules["decoder"](fused_seq, flow_inits=flow_inits)
...
def resize_to(img: np.ndarray, size: Tuple[int, int]) -> np.ndarray:
    """Resize numpy RGB image to (width, height)."""
...
def make_all3_panel(
    ...
    Bottom row: Pred Flow 1->2, Pred Flow 2->3, Pred Flow 3->4, GT Flow selected by src_idx

    Flow visualization uses the same RAFT/Middlebury color wheel functions in this file.
    """
    # imgs_seq: [T,C,H,W], pred_flows: [T-1,2,H,W]
    num_frames = imgs_seq.shape[0]
    num_flows = pred_flows.shape[0]
    if num_frames < 4 or num_flows < 3:
        raise ValueError(f"Expected 4 frames and 3 flows, got frames={num_frames}, flows={num_flows}")

    frame_imgs = []
    for t in range(4):
        frame_np = tensor_image_to_uint8(imgs_seq[t])
        frame_imgs.append(add_label(frame_np, f"Frame {t + 1}"))
    ...
    gt_label = f"GT Flow ({gt_t + 1}->{gt_t + 2})"
    gt_img = add_label(flow_to_image(gt_np), gt_label)
    flow_imgs.append(gt_img)

    # Make every tile same size as the first frame tile for clean 4-column layout.
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            for b in range(pred_flows.shape[0]):
                ...
                panel = make_all3_panel(
                    imgs_seq=imgs[b],
                    pred_flows=pred_flows[b],
                    gt_flow=gt_flow[b],
                    src_idx=src_idx[b],
