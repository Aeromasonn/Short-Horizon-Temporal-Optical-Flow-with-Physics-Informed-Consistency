...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v19: v18 backbone + multi-frame photometric loss over all predicted adjacent flows, no augmentation loss"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v19_uno_multiframe_config.json"),
...
def pair_photometric_map(
    ...
    return lambda_l1 * l1_map + lambda_ssim * ssim_map + lambda_census * census_map
...
def multiframe_photometric_loss(
    ...
    """
    imgs:  [B, T, C, H, W]
    flows: [B, T-1, 2, H, W], each flow warps imgs[:, t] toward imgs[:, t+1]

    reduction:
      - "mean": mean over all adjacent pair photometric losses
    ...
    stacked = torch.stack(pair_maps, dim=1)  # [B, T-1, H, W]

    if reduction == "min":
        return stacked.min(dim=1)[0].mean()
    if reduction == "mean":
        return stacked.mean()
...
def main():
    ...
    trainer = V19Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v19_epoch_200.pth"
