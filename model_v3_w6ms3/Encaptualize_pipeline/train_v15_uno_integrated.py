...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v15: v5 benchmark losses + Sobel encoder + partner UNO integration"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    trainer = V15Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v15_epoch_200.pth"
