...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v26 early-operator: same v26 loss/decoder, UNO before fusion"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v26_early_operator_config.json"),
...
class V26Trainer:
    ...
    def forward_pipeline(self, imgs, valid):
        ...
        # UNO is applied directly after pairwise image-flow encoding.
        # It refines the motion pair embeddings before the MotionBranchCNN and before
        # Joint Visual-Motion Fusion. All losses/decoder/training logic stay unchanged.
        valid_ds = None
...
def main():
    ...
    trainer = V26Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v26_epoch_200.pth"
