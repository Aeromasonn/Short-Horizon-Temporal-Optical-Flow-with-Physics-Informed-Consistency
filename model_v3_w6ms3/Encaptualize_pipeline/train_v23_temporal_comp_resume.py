...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v23: v22 + direct-vs-composed temporal flow consistency"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v23_temporal_comp_config.json"),
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a saved checkpoint to resume from, e.g. checkpoints/fullpipeline_v23_latest.pth",
    )
    parser.add_argument(
        "--reset_optimizer",
        action="store_true",
...
def temporal_loss(flows):
    """Legacy fixed-coordinate temporal loss. Kept only as a fallback."""
    if flows.shape[1] < 2:
        return flows.new_tensor(0.0)
    return (flows[:, 1:] - flows[:, :-1]).abs().mean()


def temporal_composition_loss(flows_adjacent, flows_skip, robust_eps=0.01):
    """Direct-vs-composed short-horizon temporal flow consistency.

    For 4 frames, adjacent flows are:
        flows_adjacent[:,0] = F01, flows_adjacent[:,1] = F12, flows_adjacent[:,2] = F23

    Direct skip predictions are expected as:
        flows_skip[:,0] = F02_direct
        flows_skip[:,1] = F13_direct

    The loss enforces:
        F02_direct ~= F01 + warp(F12, F01)
        F13_direct ~= F12 + warp(F23, F12)

    This is different from accumulated photometric loss: it compares flow fields,
    not images. It therefore gives a motion-level temporal constraint even in
    low-texture regions such as sky.
    """
    if flows_adjacent.shape[1] < 2 or flows_skip is None or flows_skip.shape[1] < 1:
        return flows_adjacent.new_tensor(0.0)

    vals = []
    max_pairs = min(flows_adjacent.shape[1] - 1, flows_skip.shape[1])
    for t in range(max_pairs):
        composed = compose_adjacent_flows(flows_adjacent[:, t], flows_adjacent[:, t + 1])
        direct = flows_skip[:, t]
        diff = direct - composed
        diff_mag = torch.sqrt(torch.sum(diff * diff, dim=1) + robust_eps * robust_eps)

        # Only remove locations where the composition samples outside the image.
        # Do not use sky/texture masks here, because this term is meant to help
        # low-texture regions through motion consistency.
        valid = warp_valid_mask(flows_adjacent[:, t])
        vals.append((diff_mag * valid).sum() / (valid.sum() + 1e-6))

    return torch.stack(vals).mean()


def build_skip2_sequence(imgs):
    """Build a 4-frame sequence that lets the unchanged full architecture predict skip flows.

    Original frames: [I0, I1, I2, I3]
    Skip sequence:   [I0, I2, I1, I3]

    The full model still outputs three adjacent flows for this artificial sequence:
        output[:,0] = I0 -> I2  (F02_direct)
        output[:,1] = I2 -> I1  (unused bridge pair)
        output[:,2] = I1 -> I3  (F13_direct)

    We use only indices 0 and 2. This keeps model dimensions unchanged and avoids
    adding a new direct-flow head.
    """
    if imgs.shape[1] < 4:
        return None
    return torch.stack([imgs[:, 0], imgs[:, 2], imgs[:, 1], imgs[:, 3]], dim=1)
...
def load_checkpoint(resume_path, modules, optimizer=None, device="cpu", reset_optimizer=False):
    ...
    if missing:
        raise KeyError(f"Checkpoint is missing module states: {missing}")

    if optimizer is not None and (not reset_optimizer) and "optimizer_state_dict" in ckpt:
        optimizer.load_state_dict(ckpt["optimizer_state_dict"])

    start_epoch = int(ckpt.get("epoch", 0))
    stats = ckpt.get("stats", {}) or {}
    best_loss = float(stats.get("loss", float("inf")))

    print(f"Resumed checkpoint from: {resume_path}")
    print(f"Checkpoint epoch: {start_epoch}")
    if reset_optimizer:
        print("Optimizer was NOT restored because --reset_optimizer was used.")
    else:
...
class V23Trainer:
    ...
    def train_step(self, batch):
        ...
        # v23: direct skip-frame flow predictions for temporal composition.
        # This is a full second pass through the same architecture, not a new head.
        # It predicts F02_direct and F13_direct from the artificial sequence [I0,I2,I1,I3].
        skip_flows_direct = None
        if self.cfg["loss"].get("temporal_use_composition", True):
            skip_imgs = build_skip2_sequence(imgs)
            if skip_imgs is not None:
                out_skip = self.forward_pipeline(skip_imgs, valid)
                skip_flows_direct = torch.stack([out_skip["flows"][:, 0], out_skip["flows"][:, 2]], dim=1)
        ...
        if self.cfg["loss"].get("temporal_use_composition", True):
            loss_temp = temporal_composition_loss(
                pred_flows,
                skip_flows_direct,
                robust_eps=self.cfg["loss"].get("temporal_comp_robust_eps", 0.01),
            )
        else:
...
def main():
    ...
    trainer = V23Trainer(cfg, modules, optimizer, writer, device)
    ...
    if args.resume is not None:
        ...
        if not resume_path.is_absolute():
            resume_path = THIS_DIR / resume_path
        start_epoch, best_loss, _ = load_checkpoint(
            resume_path, modules, optimizer, device=device, reset_optimizer=args.reset_optimizer
        )
    ...
    if start_epoch >= cfg["train"]["num_epochs"]:
        print("Checkpoint epoch {} is already >= target num_epochs {}.".format(start_epoch, cfg["train"]["num_epochs"]))
        writer.close()
        return
    ...
    for epoch in range(start_epoch, cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v23_epoch_200.pth"
