...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v26 separate-operator: same v26 loss/decoder, standalone UNO path after fusion"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v26_separate_operator_config.json"),
...
class V26Trainer:
    ...
    def forward_pipeline(self, imgs, valid):
        ...
        # The UNO branch receives the same fused image-motion sequence, but its output is
        # used as the decoder latent directly. We do NOT add it back to fused_seq.
        # This tests whether UNO alone can produce a useful operator embedding.
