...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v25: v24 + raw-RGB dilated correlation encoder"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v25_dilatedcorr_config.json"),
...
def main():
    ...
    trainer = V25Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v25_epoch_200.pth"
