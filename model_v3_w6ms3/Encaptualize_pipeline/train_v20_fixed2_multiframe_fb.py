...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v20-fixed2: bidirectional multi-frame photometric loss + soft forward-backward confidence + accumulated skip-frame loss"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v20_fixed2_multiframe_fb_config.json"),
...
class V20Fixed2Trainer:
    ...
    def train_step(self, batch):
        ...
        if self.cfg["loss"].get("photo_use_fb_consistency", True):
            ...
            if self.cfg["loss"].get("photo_bidirectional", True):
                loss_self_bw = multiframe_photometric_loss(
                    ...
                    fb_conf=torch.flip(fb_conf, dims=[1]),
                )
        ...
        loss = (
            ...
            + self.cfg["loss"].get("lambda_acc_smooth", 0.005) * loss_acc_smooth
...
def main():
    ...
    trainer = V20Fixed2Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        print(
            ...
            f"fb_valid={avg['fb_valid_ratio']:.4f}"
        )
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v20_fixed2_epoch_200.pth"
