...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v17: v15 UNO integrated backbone + boundary loss only ablation"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v17_uno_config.json"),
...
def main():
    ...
    trainer = V17Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v17_epoch_200.pth"
