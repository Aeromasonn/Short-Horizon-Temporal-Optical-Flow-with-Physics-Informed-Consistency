...
def parse_args():
    parser = argparse.ArgumentParser(
        description="v21: v20 fixed2 + explicit forward-backward consistency loss; no accumulated smoothness"
    )
    parser.add_argument(
        "--config",
        type=str,
        default=str(THIS_DIR / "v21_fb_loss_config.json"),
...
def main():
    ...
    trainer = V21Trainer(cfg, modules, optimizer, writer, device)
    ...
    for epoch in range(cfg["train"]["num_epochs"]):
        ...
        if (epoch + 1) == 200:
            save_path_200 = save_dir / "fullpipeline_v21_epoch_200.pth"
