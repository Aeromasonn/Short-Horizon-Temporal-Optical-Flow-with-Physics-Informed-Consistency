...
```
python -m Downstream_Inference --config config.json --flow_ckpt PATH_TO_FLOW_CHECKPOINT.pth --detector_ckpt PATH_TO_DETECTOR_CKPT.pt --out_dir runs --save_gt --score_thresh YOUR_THRESHOLD --no_save 
```
...
These models are based on the same training configurations as the existing `config.json` in this repository. All models are trained for 400 epochs. The checkpoints record the best training outcome throughout the 400 epochs.
...
Below is an optical flow predictions for the Standalone variant. Row 1, figures 1 and 2 (left to right) are target images; row 1, figure 3, is the estimated flow; row 2, figure 1 the ground truth flow; row 2, figure 2 is the EPE map of our estimation.
