...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable saving prediction PNGs. mAP JSON is still computed and saved.",
    )
...
def average_precision_from_pr(recalls, precisions):
    """
    Interpolated AP from precision-recall arrays.
    """
...
def compute_ap_for_iou_threshold(records, iou_thresh):
    """
    ...
    records is a list of dicts:
        {
          "image_id": str,
          "gt_boxes": Tensor [G,4],
          "pred_boxes": Tensor [P,4],
          "pred_scores": Tensor [P]
        }
    """
    total_gt = int(sum(r["gt_boxes"].shape[0] for r in records))
    total_pred = int(sum(r["pred_boxes"].shape[0] for r in records))
    ...
    for r in records:
        image_id = r["image_id"]
        gt_boxes = r["gt_boxes"].detach().cpu().float()
        pred_boxes = r["pred_boxes"].detach().cpu().float()
        pred_scores = r["pred_scores"].detach().cpu().float()

        gt_by_image[image_id] = gt_boxes
        matched_by_image[image_id] = torch.zeros((gt_boxes.shape[0],), dtype=torch.bool)
    ...
    for i, pred in enumerate(preds):
        ...
        ious = box_iou_xyxy(pred["box"].view(1, 4), gt_boxes).view(-1)
        best_iou, best_gt_idx = torch.max(ious, dim=0)
        best_gt_idx = int(best_gt_idx.item())

        if float(best_iou.item()) >= iou_thresh and not bool(matched_by_image[image_id][best_gt_idx]):
            tp[i] = 1.0
            matched_by_image[image_id][best_gt_idx] = True
        else:
            fp[i] = 1.0
    ...
    ap = average_precision_from_pr(recalls, precisions) if len(preds) > 0 else 0.0
    num_tp = int(cum_tp[-1]) if len(preds) > 0 else 0
    num_fp = int(cum_fp[-1]) if len(preds) > 0 else 0
    num_fn = int(total_gt - num_tp)
    ...
    return {
        ...
        "num_fp": num_fp,
        "num_fn": num_fn,
        "precision_final": float(precisions[-1]) if len(preds) > 0 else 0.0,
        "recall_final": float(recalls[-1]) if len(preds) > 0 else 0.0,
    }


def summarize_map(records, iou_thresholds=None):
    """
    Summarize mAP over a list of IoU thresholds.
    Default is COCO-style IoU thresholds: 0.50:0.05:0.95.
    """
    ...
    for thr in iou_thresholds:
        out = compute_ap_for_iou_threshold(records, float(thr))
        key = f"AP@{thr:.2f}"
        per_iou[key] = out
        if out["ap"] is not None:
            valid_aps.append(out["ap"])

    ap50 = per_iou.get("AP@0.50", {}).get("ap", None)
    ap75 = per_iou.get("AP@0.75", {}).get("ap", None)
    overall_map = float(np.mean(valid_aps)) if len(valid_aps) > 0 else None

    return {
        "overall": {
            "mAP_0.50_0.95": overall_map,
            "AP50": ap50,
            "AP75": ap75,
            "num_images": len(records),
            "num_gt_boxes": int(sum(r["gt_boxes"].shape[0] for r in records)),
            "num_pred_boxes": int(sum(r["pred_boxes"].shape[0] for r in records)),
        },
        "per_iou": per_iou,
    }


def save_map_json(map_summary, out_path, extra_meta=None):
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    payload = {
        "metric": "single-class bounding-box mean average precision",
        "box_format": "xyxy",
        "ap_integration": "interpolated precision-recall envelope",
        "summary": map_summary,
    }
...
def main():
    ...
    for batch_i, batch in enumerate(tqdm(loader, desc="Inference")):
        ...
        for sample_i in range(B):
            ...
            gt = batch["label"][sample_i]
            pred = result["detections"][sample_i]
            image_id = make_save_name(batch, sample_i, args.flow_index).replace(".png", "")

            map_records.append({
                "image_id": image_id,
                "gt_boxes": gt["boxes"].detach().cpu().float(),
                "pred_boxes": pred["boxes"].detach().cpu().float(),
                "pred_scores": pred["scores"].detach().cpu().float(),
            })
            evaluated += 1
    ...
    map_summary = summarize_map(map_records)
    map_json_path = out_dir / "mean_average_precision.json"
    save_map_json(
        ...
        extra_meta={
            ...
            "num_evaluated_samples": evaluated,
        },
    )

    print(f"[mAP] mAP_0.50_0.95={map_summary['overall']['mAP_0.50_0.95']}")
    print(f"[mAP] AP50={map_summary['overall']['AP50']} AP75={map_summary['overall']['AP75']}")
    print(f"[mAP] saved JSON to: {map_json_path.resolve()}")
    if args.no_save:
        print(f"[Done] evaluated {evaluated} samples; PNG saving disabled (--no_save).")
    else:
        print(f"[Done] evaluated {evaluated} samples and saved {saved} PNG files to: {out_dir.resolve()}")
